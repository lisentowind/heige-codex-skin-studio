...
- **阅读增强默认开启**：AI 回复区使用当前主题的面板色形成半透明阅读底，不在长对话区启用大面积实时模糊。主题中心可随时关闭，选择会保存在 renderer 本地并同步到其他窗口，不改变皮肤常驻状态。
...
若背景细节仍干扰 AI 回复文字，请保持主题中心的「阅读增强」开启。浅色主题会使用接近白色的半透明底，深色主题会使用深色半透明底。关闭后 AI 回复区恢复完全透明。
