...
Windows 适配已随本版本提供：双击 `scripts\windows\install.bat` 安装并默认应用 Miku 预设，日常切换同样走右上角 🎨 菜单；`pause.bat` 暂停，`customize.bat` 选图做皮肤。实现完整、单测覆盖，作者手头暂无 Windows 实机，欢迎第一批 Windows 用户开 Issue 反馈。已根据首批用户反馈修复 🎨 按钮被系统窗口控制按钮遮挡的问题：Windows 下菜单自动左移让位，并声明可点击区域。
...
支持。双击 `scripts\windows\install.bat` 安装，日常切换同样走右上角 🎨 菜单（菜单会自动避开系统窗口控制按钮）。Windows 适配代码完整且有单测覆盖，目前等待实机验收，遇到问题欢迎开 Issue。
...
Quick start: run `scripts/install.command` on macOS or `scripts\\windows\\install.bat` on Windows (new, pending live acceptance), then switch themes from the in-app menu. On Windows the menu automatically shifts clear of the native window caption buttons. Pause anytime with `scripts/pause.command`; a normal Codex restart always returns to stock.
